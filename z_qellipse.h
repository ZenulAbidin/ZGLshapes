...
namespace z_qtshapes {
    ...
    inline void ZQEllipse::normalizeAngle() noexcept
    {
        while (a < 0)
            a += 360 * MAX_ROTATIONS;
    ...
    inline int ZQEllipse::normalizedAngle() noexcept
    {
        while (a < 0)
            a += 360 * MAX_ROTATIONS;
        while (angle > 360 * MAX_ROTATIONS)
    ...
    inline void ZQEllipseF::normalizeAngle() noexcept
    {
        while (a < 0)
            a += 360 * MAX_ROTATIONS;
        while (angle > 360 * MAX_ROTATIONS)
    ...
    inline qreal ZQEllipseF::normalizedAngle() noexcept
    {
        while (a < 0)
            a += 360 * MAX_ROTATIONS;
        while (angle > 360 * MAX_ROTATIONS)
    ...
    { xp += xp1; yp += yp1; w += xp2 - xp1; h += yp2 - yp1; angle = normalizeAngle(a + angle); }

    inline void ZQEllipseF::adjustRadians(qreal xp1, qreal yp1, qreal xp2, qreal yp2, qreal angle) noexcept
    { xp += xp1; yp += yp1; w += xp2 - xp1; h += yp2 - yp1; angle = normalizeAngle(a + 180/M_PI * angle); }
    ...
    { return ZQEllipseF(xp + xp1, yp + yp1, w + xp2 - xp1, h + yp2 - yp1, normalizeAngle(a + angle)); }
    ...
    { return ZQEllipseF(xp + xp1, yp + yp1, w + xp2 - xp1, h + yp2 - yp1, normalizeAngle(a + 180/M_PI * angle)); }
